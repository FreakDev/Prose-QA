...
describe("discoverLightpandaExecutable", () => {
  it("finds binary in project .pqa/engine without explicit config path", () => {
    const cwd = mkdtempSync(path.join(tmpdir(), "pqa-lp-"));
    const engineDir = path.join(cwd, ".pqa", "engine");
    mkdirSync(engineDir, { recursive: true });
    const binary = path.join(engineDir, "lightpanda");
    writeFileSync(binary, "");
  ...
  it("prefers configured path when present", () => {
    ...
    mkdirSync(path.join(cwd, ".pqa", "engine"), { recursive: true });
    writeFileSync(path.join(cwd, ".pqa", "engine", "lightpanda"), "");
...
describe("lightpandaBrowserEnv", () => {
  it("auto-discovers project .pqa/engine when engine is lightpanda", () => {
    const cwd = mkdtempSync(path.join(tmpdir(), "pqa-lp-"));
    const engineDir = path.join(cwd, ".pqa", "engine");
    mkdirSync(engineDir, { recursive: true });
    ...
    assert.equal(env.AGENT_BROWSER_EXECUTABLE_PATH, path.join(engineDir, "lightpanda"));
